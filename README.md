...
Репозиторий содержит мою личную работу в проекте «Авитоша» (кейс 1) для хакатона Авито. Я отвечал за **фундамент Go-приложения**, **полную систему авторизации и управления сессиями**, **retention и reward-механики**, **backend мини-маркетплейса с интеграцией в прогресс питомца**, а также **production-деплой приложения в Yandex Cloud**.
...
Я реализовал backend мини-маркетплейса - не отдельную заглушку для квестов, а полноценный демонстрационный контур пользовательских действий, из которых строится игровая прогрессия.
...
- серверную интеграцию действий маркетплейса с существующей игровой транзакцией: действия меняют XP, прогресс FIRST_ROOM, награды, достижения и состояние питомца, а события публикуются только после commit;
- anti-abuse-правила: проверка владельца, запрет действий со своим объявлением и self-purchase, уникальность избранного, первого сообщения, просмотра, критерия качества и demo-сделки, а также идемпотентная обработка событий;
- unit-, handler-, repository- и migration-тесты сценариев, включая повторную demo-покупку, повторные награды и проверку авторизации.
...
flowchart LR
    ...
    Handler --> Usecase["Use case: auth, sessions, retention, marketplace"]
...
**Мини-маркетплейс делает эту связь наглядной и продуктовой.** Вместо абстрактной кнопки «выполнить квест» пользователь проходит знакомый Avito-сценарий: изучает предложения, общается с продавцом, публикует своё объявление и выбирает доставку при покупке. Каждое действие имеет самостоятельную продуктовую ценность, а игровая обратная связь - рост питомца, предметы комнаты и награды - мягко мотивирует завершать полезный путь. Поэтому решение одновременно демонстрирует, как геймификацию можно встроить в реальный продукт, и делает демо убедительнее для проверки.
...
- мини-маркетплейс: каталог, создание и публикацию объявления, избранное, сообщение продавцу и demo-покупку с доставкой;

...
Сценарий основного квеста питомца : регистрация → пять уникальных просмотров объявлений категории furniture → добавление в избранное → первое сообщение продавцу → создание и публикация собственного объявления → demo-покупка с доставкой. На каждом зачтённом действии backend возвращает `actionResult`: питомец получает XP, продвигается сюжет первой комнаты и открываются предметы DESK, LAMP, CHAIR, PLANT и POSTER.
Также полностью работает выполнение ежедневных заданий (квестов), связанных с площадкой объявлений.
